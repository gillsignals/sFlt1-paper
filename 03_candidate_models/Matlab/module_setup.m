...
% set core parameter values - for DDE candidates, use jh_opt_median_230307 (others have too few variables)
% toy: toy values for on-off case for nondimensionalizing (no epsilon, kappa)
% jh_opt_median_230307: Jung-Hornig simultaneous optimization performed 3/7/23
% hjk_opt_median_230821: Kinghorn-Jung-Hornig simultaneous optimization performed 8/21/23 (no epsilon, kappa)
...
time_interval = 1/60;   % record results every minute
...
switch param_set
    ...
    case "hjk_opt_median_230821"
        % median optimal values from Jung+Hornig optimization 8/21/23
        p.alpha = 141901;       % I production rate constant (#/cell/h)
        p.beta = .05123;       % I -> X secretion rate constant (h^-1)
        p.gamma = .12151;    % I degradation rate constant (h^-1)
        p.delta = .0574;    % X degradation rate constant (h^-1)
        p.tau = 1.96;     % I -> X secretion lag time (h)
