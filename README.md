...
- `01_ode_model/`: simulate sFLT1 trafficking as a set of ordinary differential equations
- `02_dde_model/`: simulate sFLT1 trafficking as a set of delay differential equations with fixed delay after production
- `03_candidate_models/`: compare 8 different ODE/DDE models of sFLT1 trafficking
- `04_sensitivity/`: perform sensitivity analysis and simulate chemical/genetic inhibition of the chosen DDE model

Each code folder contains simulations in `Matlab/` and analysis in `R/` subfolders.

### Other folders

- `saved-data/`: experimental datasets; input directory for R code (move desired Matlab output here)
- `saved-figs/`: output directory for figures generated by R code
...
This folder contains code to optimize 8 different systems of ordinary and delay differential equations to experimental data. 
The equations include some combination of temporal decay in production rate, delayed intracellular clearance, and internalization of extracellular sFLT1.

### `03_candidate_models/Matlab/`

#### Driver file: `driver_dde_candidate.m`

To run simulations related to Figures 3 and S3, run the driver file `driver_dde_candidate.m`. 
All other scripts in the folder are called within this driver.
...
To export output data for analysis in R, set `save_params = 1` and `save_data = 1`, which will 
save timestamped output files in `03_candidate_models/runs/`. 
To run R scripts with these data, manually move output files to `saved-data/03_candidate_models/`.
...
To run simulations related to Figures 5-8 and S9-S13, run the driver file `sens_analysis.m`. 
All other scripts in the folder are called within this driver.
...
See `saved-data/package_citations.md`.
