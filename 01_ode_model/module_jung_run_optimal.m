% simulate Jung results with optimal parameter set to store time course output
        [jung_sim_time, jung_sim_y, jung_sim_X_frac, jung_sim_I_fc] = ...
            sim_pulse_chase_ode(sp, p, pulse_length, chase_length, ...
            time_interval, conv_factor_ngml);
                         
        % store Jung courses in a single data structure
        jung_T_all(:,i) = jung_sim_time;
        jung_Y_all(:,:,i) = jung_sim_y;
        jung_X_frac_all(:,i) = jung_sim_X_frac;
        jung_I_fc_all(:,i) = jung_sim_I_fc;
        ...
        for j = 1:length(jung_v2.time)
            ...
            case 'all_pts_absolute'
                jung_X_cost_detail(i,j) = jung_sim_X_frac(tindex) - jung_v2.media(j);
                jung_I_cost_detail(i,j) = jung_sim_I_fc(tindex) - jung_v2.lysate(j);
                                       
            % (simulated - experimental)/experimental for all Sx, Si fold change    
            case 'all_pts_relative'
                if jung_v2.media(j) ~=0
                    jung_X_cost_detail(i,j) = (jung_sim_X_frac(tindex) - ...
                        jung_v2.media(j))/jung_v2.media(j);
                end
                jung_I_cost_detail(i,j) = (jung_sim_I_fc(tindex) - ...
                ...
                case 'omit_last_Sx_relative'
                    if jung_v2.time(j) ~= 10 & jung_v2.media(j) ~= 0
                        % error normalized to observed value
                        jung_X_cost_detail(i,j) = (jung_sim_X_frac(tindex) - jung_v2.media(j))/jung_v2.media(j);
                    end
                    jung_I_cost_detail(i,j) = (jung_sim_I_fc(tindex) - jung_v2.lysate(j))/jung_v2.lysate(j);
            end
