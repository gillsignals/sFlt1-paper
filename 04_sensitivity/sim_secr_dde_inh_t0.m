function [T_final, Y_final] = sim_secr_dde_inh_t0(sp, p, p_inh,...
    secr_end_time, time_interval, conv_factor, status_message)

% SIM_SECR_DDE_INH_T0  Simulate response of sFlt1 trafficking to media change 
%               plus inhibitor treatment in one cell type.
%   
% INPUT
% -----
%
% p:            structure containing initial parameter values (for
%               uninhibited part of chemical inhibition sims)
%
% p_inh:        structure containing inhibited parameter values
%
...
if ismember(status_message, [{'all'}])
    ...
    disp("X_ss = " + y0_ss(sp.Sx) + " ng/ml)")
    disp("I_ss = " + y0_ss(sp.Si) + " #/cell")
end       
...
% simulate time interval WITH NEW PARAMETER VALUES (as if drug applied at t=0)
sol = dde23(@dde_eqns, p_inh.tau, y0_ss', T_final, dde_options,...
    sp, p_inh, prod_case, conv_factor);
